function [f,s,m,mv,mvd,unA] = ...
          expmv_fun(t,Afun,b,M,prec,shift,bal,full_term,prnt)
...
if nargin < 6 || isempty(shift), shift = false; end

tAfun = @(x) t*Afun(x);

if nargin < 5 || isempty(prec), prec = 'single'; end
if nargin < 4 || isempty(M)
   tt = 1;
   [M,mvd,alpha,unA] = select_taylor_degree_fun(tAfun,b,[],[],prec,false,false);
   mv = mvd;
else
   tt = t; mv = 0; mvd = 0;
end
